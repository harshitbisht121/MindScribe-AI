...
export const LectureDetail = ({ lectureId, onBack, fetchLectures }) => {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }} className="no-print">
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: colors.text, fontWeight: 700, fontSize: 18 }}>{lecture?.title}</div>
          ...
          lecture?.transcript
            ? (
              <div style={{ background: colors.surface, borderRadius: 16, padding: 24, border: `1px solid ${colors.border}` }} className="printable-content">
                ...
                <div style={{ color: colors.text, fontSize: 14, lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{lecture.transcript}</div>
              </div>
            )
